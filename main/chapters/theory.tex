	...
	\begin{algorithm}
		...
		\myinput{training set $\mathcal{T}\subset V$;}
		...
		\nl		\ForEach{$v\in V$}{
		\nl			$h_{v}^{k} =
		\sigma\left(CONCAT\left(AGGREGATE_k(\{W^{k}\cdot h_{u}^{k-1}, u \in
		\mathcal{N}(v)\}),B^{k}\cdot h_{v}^{k-1}\right)\right)$
				}		
   			}
	...
	\cite{hamilton2017inductive}. The focus of this thesis is placed on 
	supervised classification tasks, which is why unsupervised learning
	approaches are not further investigated. In the following paragraph the 
	first GNN method of interest is presented. 
...
d_{k,k}\tilde d_{i,i}}} W^{(l)}\right)
	...
	This means that both $i$ and $j$ are neighbors of the target node $v$ at 
	layers $l$ and $(l-1)$ respectively. Linear message
	...
	Laplacian. The aggregation process differs to the one shown in algorithm 
	...
	problem for master thesis for which no perfect solution exists. As
