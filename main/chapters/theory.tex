...
d_{k,k}\tilde d_{i,i}}} W^{(l)}\right)
	...
	among others \citep{kim2012multiplicative}. While, the creation of a 
	graph which follows real-world network properties would be desirable, it is 
	not the primary target for this thesis. The main goal of the semi-synthetic graph 
	generation will be to create a graph that provides useful additional 
	...
	Kim \& Leskovec \citeyearpar[p. 138-139]{kim2012multiplicative} present 
	model parameters which generate graphs that follow real-world network 
	properties. These model parameters can however not be adopted for the task 
	at hand. The parameters were created for a somewhat simpler task, which 
	only involved randomly generated feature data. When working with real 
