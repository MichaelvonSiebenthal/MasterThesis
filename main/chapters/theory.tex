...
d_{k,k}\tilde d_{i,i}}} W^{(l)}\right)
	...
	used for training but to apply the trained GNN model to new and unseen graphs.
	GraphSage achieves this by sampling the neighborhood $\mathcal{N}(v)$ of the 
	target node $v$. This means that not the entire neighborhood of $v$ is 
	considered. Rather, only a fixed number of uniformly  sampled number of 
	neighbors $\in\mathcal{N}(v)$ are considered as inputs. A graphical example 
	of this procedure is shown in figure \ref{fig:GraphSage_sample}:
	...
	to the procedures for GCNs. The main three differences are outlined as
	follows:

	\begin{enumerate}
		\item For GraphSage only a sample of $\mathcal{N}(v)$ is considered. 
			The GCN always considers the entire neighborhood $\mathcal{N}(v)$.
		\item The aggregation strategies are more flexible in that one can
			select different strategies such as mean, pool, LSTM. Alternative 
			aggregation strategies could be considered as long as the 
			aggregation strategy is designed such that it is order invariant.
			\citep[p. 5]{hamilton2017inductive} 
		\item A mini-batch application for stochastic gradient descent was
			introduced for GraphSage. GCN in its original implementation and
			publication relies on full-batch training. The authors
			\cite[p. 8]{kipf2016semi} point out in the GCN model limitations
			that mini-batch training should be introduced for GCN in future
			work. 
	\end{enumerate}

	\noindent The pseudo code for the GraphSage forward propagation layers is 
	given in algorithm \ref{algo:GraphSage} \cite[p. 12]{hamilton2017inductive}.
	...
	\begin{algorithm}
		...
		\myinput{depth $K$; weight matrices $W^{k},\forall k \in
					\{1,\dots,K\}$;}
	...
	propagation of a trained GraphSage model. The graph layers are typically
	trained using standard optimization strategies using stochastic gradient
	descent with standard backward propagation procedures. $\mathcal{B}$ refers
	to the mini-batch taken from the set of vertices $V$ from the graph
	$G(V,E)$. Finally, algorithm \ref{algo:GraphSage} refers to the model 
	introduced by \cite{hamilton2017inductive}. The model for this thesis will 
	deviate in some aspects from the pseudo code and will be specified in the 
