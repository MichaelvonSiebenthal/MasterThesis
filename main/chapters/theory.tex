	...
	\begin{algorithm}
		...
		\nl $\mathcal{L}^{\text{train}}(\Theta)=\sum_{(z_{v},y_{v})\in\mathcal{T}}CE(y_{v},z_{v})$\;
		\nl $\mathcal{L}^{\text{validation}}(\Theta)=\sum_{(z_{v},y_{v})\notin\mathcal{T}}CE(y_{v},z_{v})$\;
...
d_{k,k}\tilde d_{i,i}}} W^{(l)}\right)
	...
	realistic random graphs which obey properties of real-world networks. These 
	properties include the emergence of a giant-connected component and a 
	power-law or $\log$-normal degree-distribution. While, the creation of a
	graph which follows real-world network properties would be desirable, it is 
	not the primary target for this thesis. 
