	...
	special form of a recurrent neural network. The weight matrix $\Phi$ lastly 
	...
	adjacency matrix with the feature matrix as the input for a recurrent
	...
	\noindent The GNN Structure outlined shows an example of a 2-hop or 3 layer GNN. 
	...
	layers similar to the possibilities of CNNs or RNNs. The defining features of
	...
	\begin{itemize}
		\item $H^{l}\in\mathcal{R}^{N \times D}$ refers to the embedding matrix
...
d_{k,k}\tilde d_{i,i}}} W^{(l)}\right)
	...
	two-layer GCN (1-hop). Returning now to compact notation, this procedure can be
	expanded for three or more GCN layers. First, the notation is further
	...
	for CNNs or RNNs using a common backward propagation function. 

	\paragraph{GraphSage} \mbox{}\\
	
	\noindent This paragraph introduces GraphSage which in many ways is a
	generalization of the GCN introduces in the previous section. There are
	however very clear differences. 
