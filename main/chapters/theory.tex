	...
	invariant and the model cannot be applied to graphs of different sizes \citep{leskovec2021lecture}. 
	One of the first methods to overcome these challenges are Graph
	Convolutional Networks and are presented in the following paragraph.
	...
	\noindent GCN are inspired by Convolutional Neural Networks (CNN) and its
	famous filtering mechanism as outlined by \cite{krizhevsky2012imagenet}.
	...
	filter with a specified size (e.g. $3\times3$ grid filter). Similarly GCN
	...
	include neighbors of $v$'s neighbors etc.). 
